...
class SwaggerToJSDoc {
  ...
  resolveDefinition (refName) {
    ...
    if (def.properties) {
      for (const v of Object.values(def.properties)) {
        if (v.$ref) {
  ...
  formatDefinitionToJSDoc (refName, indentLevel = 1, _visitingRefs = new Set()) {
    ...
    for (const [key, prop] of Object.entries(def.properties)) {
      const typeStr = this.formatType(prop, indentLevel, _visitingRefs)
      const descStr = prop.description ? ` // ${prop.description.replace(/\n/g, ' ')}` : ''
  ...
  generateJSDoc (apiInfo) {
    ...
    if (apiInfo.resRef) {
      ...
      // 处理多行缩进对齐
      const formattedReturn = returnTypeStr.replace(/\n \*/g, '\n *')
