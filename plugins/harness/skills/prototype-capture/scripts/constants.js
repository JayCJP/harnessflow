 ...
 * 视口尺寸约定
 *
 * ⚠️ pc 宽度的取值直接影响截图里文字能否看清，**不是随便定的 1920** ——
 * 托管平台（墨刀等）的分享页用「适应画布」渲染，缩放比例 = 可用宽度 ÷ 画布原生宽度
 * 且**封顶 100%**，而可用宽度 = 视口宽度 − 平台侧边面板（实测墨刀约 684px）。
 * 2026-09 实测墨刀（画布原生宽 1675px，同页面只改视口宽）：
 *
 * | 视口宽 | 缩放 | 画布实际渲染宽 |
 * |---|---|---|
 * | 1280 | 36% | 596px  ← 旧默认窗口尺寸下的结果 |
 * | 1440 | 45% | 756px |
 * | 1920 | 74% | 1236px |
 * | 2560 | 100% | 1675px（原生 1:1，文字最清晰） |
 *
 * 所以**视口要够宽**（2560 让 ≤1876px 宽的画布都按 100% 渲染），
 * 调窄反而更模糊。高度不影响缩放（已实测 1080 / 1440 / 2400 / 5000 缩放置相同），
 * 高度由截图时的内容高自适应（见 collect.screenshotPage）。
 */
const VIEWPORT = {
  pc: { width: 2560, height: 1440 },
