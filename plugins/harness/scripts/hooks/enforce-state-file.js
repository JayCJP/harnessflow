 ...
 *   - stdin 读取与决策渲染统一由 lib/hook-runner.js 承担（本文件此前自带一份 fs.readSync
 *     循环，与 state.js 的 readStdin 是两套重复实现）。
 ...
 */
const { runHook, SHELL_TOOLS } = require('../lib/hook-runner')
...
/** 受保护的状态文件列表 */
const PROTECTED_FILES = [ARTIFACT.E2E_STATE, ARTIFACT.DEV_PASS]
...
/** shell 通道的写意图模式 —— 命中任一即拒绝，其余（读取/查询）放行 */
...
/** 文件工具通道拦截的工具名 */
const FILE_WRITE_TOOLS = ['Write', 'Edit', 'write_to_file', 'replace_in_file']

/**
 * 检查目标文件是否为受保护的状态文件
 * 匹配规则: 文件名精确命中 + 路径包含 .codebuddy/plans/
...
runHook('PreToolUse', ctx => {
  ...
  if (toolName === 'apply_patch') {
    const patch = ctx.toolInput.command || ''
    const mentioned = PROTECTED_FILES.find(pf => patch.includes(pf))
    if (!mentioned) return { decision: 'allow' }
    return {
      decision: 'deny',
      reason: `State file protection: patch cannot modify ${mentioned}; use the authorized workflow script`
    }
  }
  ...
  if (SHELL_TOOLS.includes(toolName)) {
    ...
    if (AUTHORIZED_SCRIPTS.some(s => command.includes(s))) return { decision: 'allow' }

    // 只读命令 → 放行（cat / type / node -e JSON.parse 读取等）
    const hitPattern = WRITE_PATTERNS.find(p => p.test(command))
    if (!hitPattern) return { decision: 'allow' }
    ...
    return {
      ...
      reason: `State file protection (shell channel): ${mentioned} can only be mutated by ${AUTHORIZED_SCRIPTS.join(' / ')}`,
      failure: {
  ...
  // ─── 文件工具通道 ───────────────────────────────────────────
  if (!FILE_WRITE_TOOLS.includes(toolName)) return { decision: 'allow' }
  if (!filePath) return { decision: 'allow' }

  const check = isProtectedStateFile(filePath.replace(/\\/g, '/'))
  if (!check.protected) return { decision: 'allow' }
  ...
  return {
    ...
    reason: `State file protection: ${check.fileName} can only be updated by advance-phase.js`,
    failure: {
      failureType: 'state_file_violation',
